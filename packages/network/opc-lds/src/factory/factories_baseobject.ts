...
function _JSONify(self: BaseUAObject, schema: StructuredTypeSchema, pojo: any) {
    ...
    for (const field of schema.fields) {
        ...
        if (hasEnumeration(field.fieldType)) {
            ...
            if (field.isArray) {
                pojo[field.name] = fieldValue.map((value: any) => (enumeration.enumValues as any)[value.toString()]);
            } else {
                pojo[field.name] = (enumeration.enumValues as any)[fieldValue.toString()];
            }
            continue;
        }
