...
export const MenuItemGroup = (props: {id: string, label: string, items?: any[]}) => {
    return (
        <CustomTreeItem nodeId={props.id} label={props.label}>
            {props.items?.map((item: any) => <MenuItemGroup id={item.id} label={item.name} items={item.children} />)}
        </CustomTreeItem>
    )
}