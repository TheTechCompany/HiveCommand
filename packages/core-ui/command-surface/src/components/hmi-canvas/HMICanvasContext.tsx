	...
	return (
		<HMICanvasContext.Provider value={{
			...
			getDeviceOptions: (device) => {
				...
				let deviceValues = values.find((a: any) => a.devicePlaceholder?.name == device);
			...
			getDeviceConf: (device) => {
				...
				return values.find((a: any) => a.devicePlaceholder?.name == device)?.conf?.reduce((prev: any, curr: any) => ({
					...prev,
					[curr?.deviceKey?.key]: {
						min: curr?.min,
						max: curr?.max
