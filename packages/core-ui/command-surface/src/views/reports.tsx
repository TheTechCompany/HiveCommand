      ...
      <ControlGraphModal
        ...
        onSubmit={(graph) => {
          ...
          if (!graph.id) {
            ...
            client?.addChart?.(activePage, 'line-chart', graph.deviceID, graph.keyID, graph.unit, graph.timeBucket, graph.conditions, 0, 0, 8, 6, graph.totalize).then(() => {
              openModal(false);
            ...
            client?.updateChart?.(activePage, graph.id, 'line-chart', graph.deviceID, graph.keyID, graph.unit, graph.timeBucket,graph.conditions,  graph.x, graph.y, graph.w, graph.h, graph.totalize).then(() => {
              openModal(false);
              // refetchStructure?.()
              // refetchValues?.()
              setSelected(undefined)
