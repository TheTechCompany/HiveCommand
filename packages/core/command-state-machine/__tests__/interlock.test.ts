...
describe('Interlock Machine', () => {
				...
				processes: [
					{
						...
						edges: [
							{source: 'origin', target: '0.1'}
						]
					}
				]
			}, {
				requestState: async (event) => {
					if(event.state.speed = '0'){
						count++;
						if(count == 4){
							console.log(count)
							await machine.stop()
					...
					machine.state?.update(event.device, event.state)
					// console.log(event)
				}
			})

			setTimeout(() =>{
				console.log("Break free")
				 machine.state?.update('PT201', {pressure: '-0.45'})
				 machine.performOperation('PMP101', undefined, 'Start')
			}, 1000)

			setTimeout(() => {
				machine.state?.update('TK201', {level: '90'})
			}, 1500)
	...
	// 				if (event.device == "BLO701" && event.state == "true") {
	// 					pumpInterrupt = Date.now();
	// 				}

	// 				if (event.device == "PMP101" && event.state == "true") {
	// 					pumpTime = Date.now()
	// 				}
